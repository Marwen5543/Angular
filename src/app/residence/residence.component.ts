import { Component, EventEmitter, Output } from '@angular/core';
import { Residence } from '../core/models/residence';
import { Appartement } from '../core/models/appartement';
import { ApartementService } from '../apartement.service';

@Component({
  selector: 'app-residence',
  templateUrl: './residence.component.html',
  styleUrls: ['./residence.component.scss']
})
export class ResidenceComponent {
  constructor (private apartementService:ApartementService ){
    console.log(apartementService.getSameValueOf(this.listResidences,"address","Borj Cedria"));
  }
  Apprs:Appartement[]=[];
  msg=true;
  rechercher="";

  listResidences: Residence[] = [
    {
      id: 1, "name": "El fel", "address": "Borj Cedria",
      "image": "../../assets/images/R1.jpeg"
    },
    {
      id: 2, "name": "El yasmine",
      "address": "Ezzahra", "image": "../../assets/images/R2.jpg"
    },
    {
      id: 3, "name": "El Arij",
      "address": "Borj Cedria", "image": "../../assets/images/R3.jpg"
    },
    {
      id: 4, "name": "El Anber", "address": "Manzah 5",
      "image": "../../assets/images/R4.jpg"
    }
  ];

  listApartments: Appartement[] = [
    {
      "id": 1, "appartNum": 1, "floorNum": 0, "surface": 100, "terrace": "oui", "surfaceTerrace": 20, "category": "S + 1", "description": "AppartementS+ 1", "residence": this.listResidences[0]
    },
    {
      "id": 2, "appartNum": 2, "floorNum": 0, "surface": 130, "terrace": "non", "surfaceTerrace": 0, "category": "S + 2", "description": "AppartementS+ 2", "residence": this.listResidences[0]
    },
    {
      "id": 3, "appartNum": 3, "floorNum": 0, "surface": 150, "terrace": "oui", "surfaceTerrace": 30, "category": "S + 3", "description": "AppartementS+ 3", "residence": this.listResidences[1]
    }];



    showAppartement(residence:Residence){
      this.Apprs=this.listApartments.filter((a)=>a.residence==residence)
      if (this.Apprs.length==0)
      {
        this.msg=false;
      }else{this.msg=true;}
      console.log(this.Apprs)
    }

    outputresidence ($event:Residence){
      this.listResidences.map( (R)=>{
        if (R.id==$event.id){
          R.name=$event.name;
          R.address=$event.address;
          R.image=$event.image;
        }
      })
    }

    Delete(id:number){
      const index = this.listResidences.findIndex((res) => res.id === id);

    // Remove the residence from the list if found
    if (index !== -1) {
      this.listResidences.splice(index, 1);
    }
  }


  }


